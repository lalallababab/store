...
export default {
  ...
  methods: {
    ...
    //修改某一个产品的个数 使用节流 传入三个参数 第一个为type 第二个参数是操作的那个数据 第三个参数是操作的那个商品
    handler:throttle(async function (type,disNum,cart){
      switch (type) {
        case "add":
          disNum = 1;
          break;
          //  event.target.value进行加以还是减一
        case "minus":
      ...
      try {
        ...
        // 再一次获取服务器最新的数据进行展示  将数据更新后价格什么的都跟着更新
        this.getData();
      } catch (error) {
        alert(erro.message)
      }
...
//    批量删除选中商品
    ...
    //派发action 不能直接操作state中的数据 必须通过派发的action
     try{
...
,
  //  判断底部复选框是否勾选  every遍历 都满足为true 一个不满足为true
  isAllCheck()
  {
